...
begin
  ...
  try
    ...
    META_AddColumn_VARCHAR(   oTable, csDB_FLD_USR_ITEMGROUP_NAME , False {bNullable},  ciDB_FLD_USR_ITEMGROUP_NAME_Length);
  ...
  META_CreateTrigger( FIXOBJNAME(csDB_TBL_USR_ITEMGROUP),
                      ...
                      ' IS NULL) THEN NEW.' + FIXOBJNAME(csDB_FLD_ADM_X_TSPCRE) + ' = current_timestamp;' + CHR(13) + CHR(10) +
                      { INSERTING OR UPDATING - USRUPD, TSPUPD }
                      ' IF (INSERTING ' + 'OR (NEW.' + FIXOBJNAME(csDB_FLD_USR_ITEMGROUP_NAME) +
                      ' <> OLD.' + FIXOBJNAME(csDB_FLD_USR_ITEMGROUP_NAME) +
                      ') ' + ') THEN ' + 'NEW.' + FIXOBJNAME(csDB_FLD_ADM_X_USRUPD) + ' = current_user;' + CHR(13) + CHR(10) +
                      ' IF (INSERTING ' + 'OR (NEW.' + FIXOBJNAME(csDB_FLD_USR_ITEMGROUP_NAME) +
                      ' <> OLD.' + FIXOBJNAME(csDB_FLD_USR_ITEMGROUP_NAME) +
                      ') ' + ') THEN ' + 'NEW.' + FIXOBJNAME(csDB_FLD_ADM_X_TSPUPD) + ' = current_timestamp;' + CHR(13) + CHR(10) +
  ...
  if Assigned(frmPrs) then frmPrs.AddStep('Creating NON-Unique index ' + csDB_TBL_USR_ITEMGROUP + '_' + csDB_FLD_USR_ITEMGROUP_NAME);
  if Assigned(frmPrs) then Application.ProcessMessages;

  META_CreateIndex_NON_Unique(oProvider, csDB_TBL_USR_ITEMGROUP, csDB_FLD_USR_ITEMGROUP_NAME);
