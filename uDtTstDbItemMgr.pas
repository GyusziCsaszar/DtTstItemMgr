...
type
  ...
  public
    ...
    function ADM_DoDbUpdates_internal(oProvider: TDBXDataExpressMetaDataProvider; var rbDoAll_Asked: Boolean; var rbDoAll: Boolean; frmPrs: TFrmProgress; ADMIN_MODE: Boolean) : Boolean; override;

    procedure ADM_CreateTable_ITEM_ITEMGROUP(oProvider: TDBXDataExpressMetaDataProvider; frmPrs: TFrmProgress);
...
function TDtTstDbItemMgr.ADM_DoDbUpdates_internal(oProvider: TDBXDataExpressMetaDataProvider; var rbDoAll_Asked: Boolean; var rbDoAll: Boolean; frmPrs: TFrmProgress; ADMIN_MODE: Boolean) : Boolean;
begin
  Result := False; // Indicated that there are more Database Updates pending...

  // Updating ADM tables...
  if not inherited ADM_DoDbUpdates_internal(oProvider, rbDoAll_Asked, rbDoAll, frmPrs, ADMIN_MODE) then
  begin
    ...
    begin
      ...
      case ADM_DbInfVersion_PRD of
        ...
        103: begin
          ...
          if Assigned(frmPrs) then frmPrs.AddStepHeader('Updating to PRD Version v1.04');

          ADM_CreateTable_ITEM_ITEMGROUP(oProvider, frmPrs);
...
begin
  ...
  else if FIXOBJNAME(sTable) = FIXOBJNAME(csDB_TBL_USR_ITEM_ITEMGROUP) then
  begin

    // ATTN: Deleting Table also deletes its Triggers!!!
    // oProvider.Execute(m_oLog.LogSQL('DROP TRIGGER ' + FIXOBJNAME(csDB_TBL_ADM_USERS) + '_BI'));

    META_DropGenerator(oProvider, csDB_TBL_USR_ITEM_ITEMGROUP, csDB_FLD_ADM_X_ID);

    // NOTE: admUsers table added in v102!
    ADM_Update_DBINFO_VER_PRD(nil {nil = DO Transaction}, 103);
...
begin
  ...
  begin
    ...
    oProvider.Execute( m_oLog.LogSQL('DROP VIEW ' + 'V_' + FIXOBJNAME(csDB_TBL_USR_ITEM) + '_EX' + ';'));
...
procedure TDtTstDbItemMgr.ADM_CreateTable_ITEM_ITEMGROUP(oProvider: TDBXDataExpressMetaDataProvider; frmPrs: TFrmProgress);
var
...
begin
  ...
  try
    ...
    if Assigned(frmPrs) then frmPrs.AddStep('Creating table ' + csDB_TBL_USR_ITEM_ITEMGROUP);
    if Assigned(frmPrs) then Application.ProcessMessages;

    oTable.TableName := FIXOBJNAME(csDB_TBL_USR_ITEM_ITEMGROUP);
    ...
    META_AddColumn_INT32(     oTable, csDB_TBL_USR_ITEM_ITEMGROUP_ITEM_ID      , False  {bNullable});

    META_AddColumn_INT32(     oTable, csDB_TBL_USR_ITEM_ITEMGROUP_ITEMGROUP_ID , False  {bNullable});
  ...
  if Assigned(frmPrs) then frmPrs.AddStep('Creating Foreign Key constraint FK_' + csDB_TBL_USR_ITEM_ITEMGROUP +  '_' + csDB_TBL_USR_ITEM_ITEMGROUP_ITEM_ID);
  if Assigned(frmPrs) then Application.ProcessMessages;

  META_CreateConstraint_ForeignKey(oProvider,
          csDB_TBL_USR_ITEM_ITEMGROUP,  csDB_TBL_USR_ITEM_ITEMGROUP_ITEM_ID,
          csDB_TBL_USR_ITEM,            csDB_FLD_ADM_X_ID);
  ...
  if Assigned(frmPrs) then frmPrs.AddStep('Creating Foreign Key constraint FK_' + csDB_TBL_USR_ITEM_ITEMGROUP +  '_' + csDB_TBL_USR_ITEM_ITEMGROUP_ITEMGROUP_ID);
  if Assigned(frmPrs) then Application.ProcessMessages;

  META_CreateConstraint_ForeignKey(oProvider,
          csDB_TBL_USR_ITEM_ITEMGROUP,  csDB_TBL_USR_ITEM_ITEMGROUP_ITEMGROUP_ID,
          csDB_TBL_USR_ITEMGROUP,       csDB_FLD_ADM_X_ID);
  ...
  if Assigned(frmPrs) then frmPrs.AddStep('Creating generator ' + csDB_TBL_USR_ITEM_ITEMGROUP +  '_' + csDB_FLD_ADM_X_ID);
  if Assigned(frmPrs) then Application.ProcessMessages;

  META_CreateGenerator(oProvider, csDB_TBL_USR_ITEM_ITEMGROUP, csDB_FLD_ADM_X_ID);
  ...
  if Assigned(frmPrs) then frmPrs.AddStep('Creating trigger ' + csDB_TBL_USR_ITEM_ITEMGROUP + '_BI');
  if Assigned(frmPrs) then Application.ProcessMessages;

  META_CreateTrigger( FIXOBJNAME(csDB_TBL_USR_ITEM_ITEMGROUP),
                      FIXOBJNAME(csDB_TBL_USR_ITEM_ITEMGROUP) + '_BI',
                      ...
                      FIXOBJNAME(csDB_TBL_USR_ITEM_ITEMGROUP) +
                      ...
                      ' IF (INSERTING ' + 'OR (NEW.' + FIXOBJNAME(csDB_TBL_USR_ITEM_ITEMGROUP_ITEM_ID) +
                      ' <> OLD.' + FIXOBJNAME(csDB_TBL_USR_ITEM_ITEMGROUP_ITEM_ID) +
  ...
  if Assigned(frmPrs) then frmPrs.AddStep('Creating Primary Key index ' + csDB_TBL_USR_ITEM_ITEMGROUP + '_' + csDB_FLD_ADM_X_ID);
  if Assigned(frmPrs) then Application.ProcessMessages;

  META_CreateIndex_PrimaryKey(oProvider, csDB_TBL_USR_ITEM_ITEMGROUP, csDB_FLD_ADM_X_ID);
  ...
  if Assigned(frmPrs) then frmPrs.AddStep('Creating NON-Unique index ' + csDB_TBL_USR_ITEM_ITEMGROUP + '_' + csDB_TBL_USR_ITEM_ITEMGROUP_ITEM_ID);
  if Assigned(frmPrs) then Application.ProcessMessages;

  META_CreateIndex_NON_Unique(oProvider, csDB_TBL_USR_ITEM_ITEMGROUP, csDB_TBL_USR_ITEM_ITEMGROUP_ITEM_ID);
  ...
  if Assigned(frmPrs) then frmPrs.AddStep('Creating NON-Unique index ' + csDB_TBL_USR_ITEM_ITEMGROUP + '_' + csDB_TBL_USR_ITEM_ITEMGROUP_ITEMGROUP_ID);
  if Assigned(frmPrs) then Application.ProcessMessages;

  META_CreateIndex_NON_Unique(oProvider, csDB_TBL_USR_ITEM_ITEMGROUP, csDB_TBL_USR_ITEM_ITEMGROUP_ITEMGROUP_ID);
  ...
  if Assigned(frmPrs) then frmPrs.AddStep('Creating view ' + 'V_' + csDB_TBL_USR_ITEM + '_EX');
  if Assigned(frmPrs) then Application.ProcessMessages;

  oProvider.Execute(
      m_oLog.LogSQL('CREATE VIEW ' + 'V_' + FIXOBJNAME(csDB_TBL_USR_ITEM) + '_EX' +
                    ' AS SELECT ' + 'A.' + FIXOBJNAME(csDB_FLD_USR_ITEM_ITEMNR) +
                             ...
                             ', ' + 'D.' + FIXOBJNAME(csDB_FLD_USR_ITEMGROUP_NODE) +
                       ' FROM ' + FIXOBJNAME(csDB_TBL_USR_ITEM) + ' A' +
                           ', ' + FIXOBJNAME(csDB_TBL_USR_ITEMTYPE) + ' B' +
                           ', ' + FIXOBJNAME(csDB_TBL_USR_ITEM_ITEMGROUP) + ' C' +
                           ', ' + FIXOBJNAME(csDB_TBL_USR_ITEMGROUP) + ' D' +
                       ' WHERE (' + 'A.' + FIXOBJNAME(csDB_FLD_USR_ITEM_ITEMTYPE_ID) + ' = ' + 'B.' + FIXOBJNAME(csDB_FLD_ADM_X_ID) + ')' +
                         ' AND (' + 'A.' + FIXOBJNAME(csDB_FLD_ADM_X_ID) + ' = ' + 'C.' + FIXOBJNAME(csDB_TBL_USR_ITEM_ITEMGROUP_ITEM_ID) + ')' +
                         ' AND (' + 'D.' + FIXOBJNAME(csDB_FLD_ADM_X_ID) + ' = ' + 'C.' + FIXOBJNAME(csDB_TBL_USR_ITEM_ITEMGROUP_ITEMGROUP_ID) + ')' +
                       ';'));

  META_CreateTrigger( 'V_' + FIXOBJNAME(csDB_TBL_USR_ITEM) + '_EX',
                      'V_' + FIXOBJNAME(csDB_TBL_USR_ITEM) + '_EX' + '_DELETE',
  ...
  META_CreateTrigger( 'V_' + FIXOBJNAME(csDB_TBL_USR_ITEM) + '_EX',
                      'V_' + FIXOBJNAME(csDB_TBL_USR_ITEM) + '_EX' + '_UPDATE',
  ...
  META_CreateTrigger( 'V_' + FIXOBJNAME(csDB_TBL_USR_ITEM) + '_EX',
                      'V_' + FIXOBJNAME(csDB_TBL_USR_ITEM) + '_EX' + '_INSERT',
                      ' ACTIVE BEFORE INSERT' + CHR(13) + CHR(10) +
                    //' POSITION 0' + CHR(13) + CHR(10) +
                      ' AS' + CHR(13) + CHR(10) +
                      '   DECLARE VARIABLE ItmGrpID BIGINT;' + CHR(13) + CHR(10) +
  ...
  try
    ...
    if Assigned(frmPrs) then frmPrs.AddStep('Inserting table ' + csDB_TBL_USR_ITEM_ITEMGROUP + ' into table ' + csDB_TBL_ADM_TABLES);
    if Assigned(frmPrs) then Application.ProcessMessages;

    ADM_UpdateOrInsert_NewTable(m_oCon {nil = DO Transaction}, csDB_TBL_USR_ITEM_ITEMGROUP);
